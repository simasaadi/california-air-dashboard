...
# Pick trend pollutant (more cross-month variance) and use 3-month SMA
...
def per_month_tables(m):
    ...
    # Regional lines (cumulative to m) + SMA3
...
# Heatmap trace (keep axes/coloraxis fixed every frame)
fig.add_trace(
    go.Heatmap(z=C0, x=corr_vars, y=corr_vars, zmin=-1, zmax=1,
               coloraxis="coloraxis", showscale=True, hoverongaps=False),
    row=1, col=2
)
...
fig.update_layout(
    ...
    # Keep UI state stable across frames
    uirevision="const"
)

# ---------- Frames: FULL traces per frame (robust, no errors) ----------
frames = []
for m in months:
    ...
    if len(cm):
        ...
        frame_traces.append(go.Scattermapbox(lat=[], lon=[], showlegend=False))

    # 1) Heatmap (full trace but x/y/coloraxis fixed so it won't jitter)
    frame_traces.append(go.Heatmap(z=C, x=corr_vars, y=corr_vars, zmin=-1, zmax=1,
                                   coloraxis="coloraxis", showscale=True, hoverongaps=False))

    # 2-4) Regional lines
    for reg in regions:
        sub = rs[rs["Region"]==reg].sort_values("_date")
        frame_traces.append(go.Scatter(x=sub["_date"], y=sub[f"{trend_pol}_SMA3"], mode="lines"))

    # 5) Surface
    frame_traces.append(go.Surface(z=Z, x=xmid, y=ymid, showscale=False))

    frames.append(go.Frame(name=m, data=frame_traces,
