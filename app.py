...
h1{margin:0.2rem 0 0.6rem 0; text-align:center; line-height:1.15;}
hr{margin:0.3rem 0 0.9rem 0; border:1px solid #d0d0d0;}
</style>
...
# Auto-pick trend pollutant (more month-to-month variance)
...
def per_month_tables(m):
    ...
    # regional (cumulative) + 3-month SMA
...
# ---------- Create figure & STATIC traces once ----------
fig = make_subplots(
    rows=2, cols=2,
...
# 0) Map (trace index 0)
if len(cm0)==0:
    lat0, lon0, size0, col0 = [], [], [], []
else:
    lat0, lon0 = cm0["lat"].tolist(), cm0["lon"].tolist()
    size0 = cm0["size"].tolist()
    col0  = cm0["Composite"].tolist()
fig.add_scattermapbox(
    lat=lat0, lon=lon0, mode="markers", showlegend=False,
    marker=dict(size=size0, color=col0, colorscale="Blues", showscale=False),
    text=(cm0["County"].tolist() if len(cm0) else []),
    hovertemplate="County=%{text}<br>NO₂/CO shown in composite<extra></extra>",
    row=1, col=1
)
fig.update_mapboxes(row=1, col=1, style="open-street-map",
                    center=dict(lat=36.5, lon=-119.5), zoom=4.5)

# 1) Heatmap (trace index 1) — coloraxis with fixed scale; we will update z only
fig.add_heatmap(z=C0, x=corr_vars, y=corr_vars, zmin=-1, zmax=1,
                coloraxis="coloraxis", showscale=True, row=1, col=2)

# 2–4) Regional lines (trace index 2..4) — one per region; we will update y only
for reg in regions:
...
# 5) Surface (trace index 5) — we will update z only
fig.add_surface(z=Z0, x=x0, y=y0, showscale=False, row=2, col=2)

# Keep axes/layout stable between frames (no blinking)
fig.update_layout(
    height=860, margin=dict(l=10, r=10, t=60, b=70),
    coloraxis=dict(colorscale="Blues", colorbar=dict(title="r", y=0.88)),
    ...
    title=dict(text=f"Month: {m0}", x=0.5, y=0.995),
    uirevision="lock"
)
fig.update_xaxes(matches=None, row=2, col=1)  # independent
fig.update_yaxes(matches=None, row=2, col=1)

# ---------- Frames using array-only updates in EXACT trace order ----------
frames = []
for m in months:
    ...
    # map arrays
    if len(cm):
        lat = cm["lat"].tolist(); lon = cm["lon"].tolist()
        size = cm["size"].tolist(); comp = cm["Composite"].tolist()
    else:
        lat=lon=size=comp=[]

    # regional y arrays in order North, Central, South
    yN = rs[rs["Region"]=="North"].sort_values("_date")[f"{trend_pol}_SMA3"].tolist()
    yC = rs[rs["Region"]=="Central"].sort_values("_date")[f"{trend_pol}_SMA3"].tolist()
    yS = rs[rs["Region"]=="South"].sort_values("_date")[f"{trend_pol}_SMA3"].tolist()

    frames.append(go.Frame(
        name=m,
        data=[
            # 0 map (update arrays only)
            {"lat":lat, "lon":lon, "marker":{"size":size, "color":comp}},
            # 1 heatmap (update z only)
            {"z":[[float(v) for v in row] for row in C]},
            # 2..4 lines (update y only)
            {"y":yN}, {"y":yC}, {"y":yS},
            # 5 surface (update z only)
            {"z":Z},
        ],
        layout=go.Layout(title=dict(text=f"Month: {m}", x=0.5, y=0.995))
    ))
fig.frames = frames
...
fig.update_layout(
    ...
    sliders=[{
        ...
        "pad":{"t":40, "b":0},
        "steps":[{"args":[[m], {"frame":{"duration":0,"redraw":False}, "transition":{"duration":0}}],
